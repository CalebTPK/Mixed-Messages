const villainData = {
    ...
    villianTitle: [
        'Doctor', 'The Diabolical', 'Count', 'The Phantom', 'Lord', 'Professor', 'Captain', 'The Malevolent', 
        'The Nefarious', 'King', 'Queen', 'Slimy', 'Darth', 'Lady', 'Green'
    ],
    villainName: [
        'Diddle.', 'Bumfuzzle.', 'Mugwump.', 'Pussyfoot.', 'Sphincter.', 'Fartlek.', 'Buttress.', 'Gardyloo.',
        'Flibbertigibbet.', 'Snollygoster.', 'Scuttlebutt.', 'Tater Tot.', 'Collywobbles.', 'Snickersnee.', 'Piffle.'
    ],
    evilPlans: [
...
const generateVillainMonologue = () => {
    ...
    for (key in villainData) {
        if (villainData.hasOwnProperty(key)) {
            const randomIndex = Math.floor(Math.random() * villainData[key].length);
            monologueParts.push(villainData[key][randomIndex]);
        }
    }

    const villainMonologue = monologueParts.join(' ');
    console.log(villainMonologue);
}

generateVillainMonologue()
